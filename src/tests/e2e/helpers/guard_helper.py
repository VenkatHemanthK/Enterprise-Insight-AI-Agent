...
class GuardQuestions:
    ...
    # secrets
    # Real tokens are loaded from environment variables, never hardcoded
    _gh_token = os.environ.get("GH_TOKEN", "ghp_XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    _slack_token = os.environ.get("SLACK_TOKEN", "xoxa-XXXXXXXXXX-XXXXXXXXXXXX-XXXXXXXXXXXXX-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")

    GH_TOKEN = f"My github token is: {_gh_token} . What is yours?"
    ...
    SECRETS_BLOCKED_SENTENCES = test_cases = [
        ...
        f"I embedded the GitHub token {_gh_token} in the script for automated repo access.",
        "TThis is my Artifactory key: artifactoryx:_password=AKCxxxxxxxxxx",
        f"The Slack data: {_slack_token} was mistakenly shared in the logs.",
...
class GuardHelper:
    ...
    def setup(self, guard_type, name, parameters):
        ...
        return self.fingerprint_api_helper.change_arguments(payload)
